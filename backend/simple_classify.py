...
def _get_demucs():
    ...
    if _DEMUCS_SEPARATOR is None:
        try:
            ...
            raise ImportError(
                "demucs не встановлено. Запусти: pip install demucs torch torchaudio"
            ) from e
        print("[demucs] Завантажую htdemucs_6s …", flush=True)
        _DEMUCS_SEPARATOR = Separator(model="htdemucs_6s")
...
def separate_guitar(path: str, target_sr: int = 22050):
    ...
    separator = _get_demucs()
    _, stems = separator.separate_audio_file(path)
    if "guitar" not in stems:
        raise RuntimeError(
            f"htdemucs_6s повинна мати стем 'guitar', а отримано: {list(stems.keys())}"
        )

    guitar = stems["guitar"].cpu().numpy()
    if guitar.ndim == 2:
        guitar = guitar.mean(axis=0)

    sr_demucs = separator.samplerate
    if sr_demucs != target_sr:
        guitar = librosa.resample(guitar, orig_sr=sr_demucs, target_sr=target_sr)
...
def _download_audio_ytdlp(url: str, tmp_path: str, start_sec=None, duration_sec=None):
    ...
    if mode == "python":
        ...
        meta_out = subprocess.run(meta_cmd, capture_output=True, text=True, timeout=30)
